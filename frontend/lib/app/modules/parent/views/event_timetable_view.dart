...
class EventTimetableView extends GetView<EventTimetableController> {
  const EventTimetableView({super.key});

  @override
  Widget build(BuildContext context) {
    final isDark = Theme.of(context).brightness == Brightness.dark;
    return Scaffold(
      appBar: CustomAppBar(title: 'Event Timetable', showBack: true),
      body: Obx(() {
              ...
              child: controller.isLoading.value
                  ...
                  : controller.errorMessage.value.isNotEmpty
                      ...
                      : controller.events.isEmpty
                          ? const Center(child: Text('No event timetable available.'))
                          : ListView.separated(
                              padding: const EdgeInsets.all(16),
                              itemCount: controller.events.length,
                              separatorBuilder: (_, __) => const SizedBox(height: 10),
                              itemBuilder: (_, i) {
                                final e = controller.events[i];
                                return Container(
                                  ...
                                  child: Column(
                                    ...
                                    children: [
                                      Text((e['title'] ?? e['name'] ?? 'Event').toString(),
                                          style: const TextStyle(fontWeight: FontWeight.bold)),
                                      const SizedBox(height: 6),
                                      Text('Date: ${(e['date'] ?? e['eventDate'] ?? '-').toString()}'),
                                      Text('Time: ${(e['time'] ?? e['startTime'] ?? '-').toString()}'),
                                      Text('Location: ${(e['location'] ?? e['venue'] ?? '-').toString()}'),
                                    ],
                                  ),
                                );
                              },
